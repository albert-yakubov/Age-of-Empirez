...
import com.stepasha.ageofempirez.dummy.DummyContent
...
class ItemDetailFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val item = arguments?.getSerializable(ARG_ITEM_ID) as API

        item_detail.text = item.name + "\n" + item.description()
        item_detail.setOnClickListener {
            listener?.onItemFragmentInteractionListener(item)
        }
    }
    ...
    interface OnItemFragmentInteractionListener {
        fun onItemFragmentInteractionListener(item: API)
    }
